...
- Start every new feature, fix, refactor, docs, or chore branch from the **main workspace's latest `master`**, not from `develop` or another worktree.
...
- **All code changes must be done inside a worktree**; never implement directly in the main workspace.
- Create each worktree from the **main workspace's latest `master`** so the local merge target stays consistent. Do not start worktrees from `origin/master`, `develop`, or another worktree unless explicitly instructed.
...
- After a branch is merged back into the main workspace's `master`, remove its corresponding worktree and delete the local branch.
