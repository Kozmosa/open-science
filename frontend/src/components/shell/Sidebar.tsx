...
export function Sidebar({ routes, collapsed, onCollapsedChange }: SidebarProps) {
  const t = useT();
  return (
    <aside
      className={cn(
    ...
    >
      ...
      {collapsed ? null : (
        <p className="border-t border-[var(--osci-color-border)] px-3 py-3 text-[11px] leading-relaxed text-[var(--osci-color-text-muted)]">
          {t('common.builtBy')}
        </p>
      )}
    </aside>
