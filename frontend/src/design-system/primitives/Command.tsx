...
export function CommandInput({ className, ...props }: React.ComponentPropsWithoutRef<typeof CommandPrimitive.Input>) {
  return (
    <div className="flex items-center border-b border-[var(--osci-color-border)] px-3">
      ...
      <CommandPrimitive.Input className={cn('h-11 w-full bg-transparent text-sm outline-none focus-visible:outline-none focus-visible:ring-0 placeholder:text-[var(--osci-color-text-muted)] disabled:opacity-50', className)} {...props} />
    </div>
  );
}
