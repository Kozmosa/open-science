...
import { getProjectTasks, getTaskEdges, moveTask } from '@features/tasks';
          ...
          <Card data-testid="project-list" className="order-2 lg:order-1"><CardBody className="space-y-2 p-3">
            {projects.map((project) => (
              <button key={project.project_id} type="button" onClick={() => setRouteState({ project: project.project_id, tab })} className={`w-full rounded-[var(--osci-radius-md)] border p-3 text-left ${selectedProject?.project_id === project.project_id ? 'border-[var(--osci-color-primary-border)] bg-[var(--osci-color-primary-soft)]' : 'border-[var(--osci-color-border-subtle)] hover:bg-[var(--osci-color-surface-subtle)]'}`}>
                <span className="flex items-center justify-between gap-2"><span className="truncate font-semibold text-[var(--osci-color-text)]">{project.name}</span>{project.attention_required ? <StatusBadge tone="warning">{t('pages.projects.attention')}</StatusBadge> : null}</span>
                <span className="mt-1 block text-xs text-[var(--osci-color-text-muted)]">{t('pages.projects.projectCounts', { running: project.running_task_count, workspaces: project.workspace_count })}</span>
          ...
          {selectedProject ? (
            <div data-testid="project-summary" className="order-1 min-w-0 space-y-4 lg:order-2">
              ...
              <Tabs value={tab} onValueChange={(value) => setRouteState({ tab: value })}>
                <TabsList className="flex w-full overflow-x-auto"><TabsTrigger value="overview">{t('pages.projects.tabs.overview')}</TabsTrigger><TabsTrigger value="tasks">{t('pages.projects.tabs.tasks')}</TabsTrigger><TabsTrigger value="workspaces">{t('pages.projects.tabs.workspaces')}</TabsTrigger><TabsTrigger value="context">{t('pages.projects.tabs.context')}</TabsTrigger><TabsTrigger value="settings">{t('pages.projects.tabs.settings')}</TabsTrigger></TabsList>
                <TabsContent value="overview"><Card><CardBody className="grid gap-4 p-5 sm:grid-cols-2 xl:grid-cols-4"><Metric label={t('pages.projects.metrics.activeTasks')} value={selectedProject.active_task_count} /><Metric label={t('pages.projects.metrics.runningTasks')} value={selectedProject.running_task_count} /><Metric label={t('pages.projects.metrics.workspaces')} value={selectedProject.workspace_count} /><Metric label={t('pages.projects.metrics.executable')} value={selectedProject.executable_workspace_count} /><div className="sm:col-span-2 xl:col-span-4"><p className="text-sm font-medium text-[var(--osci-color-text)]">{t('pages.projects.attention')}</p><p className="mt-1 text-sm text-[var(--osci-color-text-secondary)]">{attentionReasonLabels.join(' ') || t('pages.projects.noActionRequired')}</p></div></CardBody></Card></TabsContent>
                <TabsContent value="tasks">
                  <ViewToolbar><div className="flex gap-2"><Button size="sm" variant={view === 'list' ? 'primary' : 'secondary'} onClick={() => setRouteState({ view: 'list' })}>{t('pages.projects.views.list')}</Button><Button size="sm" variant={view === 'graph' ? 'primary' : 'secondary'} onClick={() => setRouteState({ view: 'graph' })}>{t('pages.projects.views.graph')}</Button></div></ViewToolbar>
                  {view === 'graph' ? <div className="mt-3 h-[620px] overflow-hidden rounded-[var(--osci-radius-lg)] border border-[var(--osci-color-border-subtle)] bg-[var(--osci-color-surface)]"><ProjectCanvas key={`${projectId}:${layoutVersion}`} projectId={projectId!} tasks={tasks} edges={edges} projects={eligibleTargetProjects.map(asCanvasProject)} onNodeClick={(taskId) => navigate(`/tasks?task=${encodeURIComponent(taskId)}`)} onNewTask={() => setTaskCreateOpen(true)} onResetLayout={() => { localStorage.removeItem(`openscience:project-layout:${projectId}`); setLayoutVersion((value) => value + 1); }} onMoveTaskToProject={(taskId, targetProjectId) => { void moveTaskToProject(taskId, targetProjectId); }} canCreateTask={canCreateTask} canEditRelationships={selectedProject.status === 'active' && selectedProject.permissions.can_edit} canMoveTask={canMoveTask} /></div> : <Card className="mt-3"><CardBody className="divide-y divide-[var(--osci-color-border-subtle)] p-0">{tasks.map((task) => <button key={task.task_id} type="button" onClick={() => navigate(`/tasks?task=${encodeURIComponent(task.task_id)}`)} className="flex w-full items-center justify-between gap-3 p-4 text-left hover:bg-[var(--osci-color-surface-subtle)]"><div><p className="font-medium text-[var(--osci-color-text)]">{task.title}</p><p className="text-xs text-[var(--osci-color-text-muted)]">{task.task_id}</p></div><StatusBadge tone={task.status === 'running' ? 'success' : task.status === 'failed' ? 'danger' : 'neutral'}>{task.status}</StatusBadge></button>)}{tasks.length === 0 ? <EmptyState message={t('pages.projects.noTasksInProject')} /> : null}</CardBody></Card>}
                </TabsContent>
