...
export default function ProjectContextConsole({ project }: { project: DomainProjectProjection }) {
  ...
  return <div className="space-y-4">
    ...
    <Card><CardBody className="space-y-3 p-5"><div><h3 className="font-semibold text-[var(--osci-color-text)]">Candidates</h3><p className="text-sm text-[var(--osci-color-text-secondary)]">Accept appends a candidate into the editable Draft; reject preserves its audit record.</p></div>{candidates.map((candidate) => <div key={candidate.candidate_id} className="rounded-[var(--osci-radius-md)] border border-[var(--osci-color-border-subtle)] p-3"><div className="flex flex-wrap items-center justify-between gap-2"><div className="flex items-center gap-2"><Badge variant="outline">{candidate.status}</Badge><span className="text-xs font-mono text-[var(--osci-color-text-muted)]">{candidate.candidate_id}</span></div>{candidate.status === 'proposed' ? <div className="flex gap-2"><Button size="sm" variant="secondary" disabled={!project.permissions.can_edit} onClick={() => rejectMutation.mutate(candidate.candidate_id)}>Reject</Button><Button size="sm" disabled={!project.permissions.can_edit} onClick={() => acceptMutation.mutate(candidate.candidate_id)}>Accept</Button></div> : null}</div><p className="mt-2 whitespace-pre-wrap text-sm text-[var(--osci-color-text-secondary)]">{candidate.content}</p></div>)}{candidates.length === 0 ? <EmptyState message="No proposed Context candidates." /> : null}</CardBody></Card>
  </div>;
}
