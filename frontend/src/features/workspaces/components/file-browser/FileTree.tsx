      ...
      {entry.kind === 'directory' && expanded && (
        <div className="ml-4 border-l border-[var(--border)] pl-1">
          {loading ? (
            ...
            <p className="px-2 py-1 text-xs text-[var(--text-tertiary)]">{t('pages.runs.fileBrowser.emptyDirectory')}</p>
          )}
        </div>
      )}
    </div>
...
export default function FileTree({ entries, selectedPath, onSelectFile, onLoadDirectory }: FileTreeProps) {
  const t = useT();
  if (entries.length === 0) {
    return <p className="px-3 py-2 text-sm text-[var(--text-tertiary)]">{t('pages.runs.fileBrowser.noFiles')}</p>;
  }
