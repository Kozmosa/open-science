...
def prepare_v2_test_state(state_root: Path) -> str:
    ...
    expensive.  Each pytest worker builds it once; individual tests receive an
    ...
    connection = {
        ...
        "default_workdir": str(
            ApiConfig(
                api_key_hashes=frozenset(), state_root=state_root
            ).runtime_paths.ensure_default_workspace_dir()
        ),
    }
    with closing(connect(state_root / "runtime" / "agentic_researcher.sqlite3")) as conn:
        conn.execute(
            ...
            INSERT OR REPLACE INTO environments (
                ...
                connection_fingerprint, disabled_at, disabled_reason
            ) VALUES (?, ?, NULL, ?, NULL, ?, NULL, 1, 'active', ?, ?, ?, NULL, NULL)
            """,
            (
                "env-localhost",
                "localhost",
