...
class TestMaintenanceBarrierRepair:
    ...
    def test_legacy_schema_marker_recreates_missing_maintenance_barrier(
        ...
        with _connect(db_file) as conn:
            assert current_version(conn, "agentic_researcher") == 29
            tables = {
                str(row["name"])
    ...
    def test_rejects_schema_versions_newer_than_this_binary(self, tmp_path: Path) -> None:
        ...
        with _connect(db_file) as conn:
            ...
            set_version(conn, "agentic_researcher", 30)
            conn.commit()

        with _connect(db_file) as conn:
            with pytest.raises(
                RuntimeError, match="unsupported agentic_researcher schema version 30"
            ):
                run_pending(conn, "agentic_researcher")
