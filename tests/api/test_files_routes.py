...
async def test_list_files_localhost(tmp_path: Path) -> None:
    ...
    async with make_client(tmp_path) as client:
        response = await client.get("/api/files/list?environment_id=env-localhost&path=")
...
async def test_list_files_not_found(tmp_path: Path) -> None:
    async with make_client(tmp_path) as client:
        response = await client.get(
            "/api/files/list?environment_id=env-localhost&path=/nonexistent"
        )
...
async def test_read_file_localhost(tmp_path: Path) -> None:
    ...
    async with make_client(tmp_path) as client:
        response = await client.get("/api/files/read?environment_id=env-localhost&path=hello.txt")
...
async def test_read_file_with_language(tmp_path: Path) -> None:
    ...
    async with make_client(tmp_path) as client:
        response = await client.get("/api/files/read?environment_id=env-localhost&path=main.py")
...
async def test_read_file_not_found(tmp_path: Path) -> None:
    async with make_client(tmp_path) as client:
        response = await client.get("/api/files/read?environment_id=env-localhost&path=missing.txt")
...
async def test_read_file_too_large(tmp_path: Path) -> None:
    ...
    async with make_client(tmp_path, max_file_size_bytes=1_048_576) as client:
        response = await client.get("/api/files/read?environment_id=env-localhost&path=big.bin")
...
async def test_stream_file_localhost_sets_frame_options(tmp_path: Path) -> None:
    ...
    async with make_client(tmp_path) as client:
        response = await client.get(
            "/api/files/stream?environment_id=env-localhost&path=report.pdf"
        )
