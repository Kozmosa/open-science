...
def _serialize_domain_environment(environment: dict[str, object]) -> EnvironmentResponse:
    ...
    if domain is not None:
        ...
        try:
            return EnvironmentListResponse(
                items=[
                    _serialize_domain_environment(environment)
...
async def update_environment(
    ...
    if domain is not None:
        ...
        try:
            ...
            environment = domain.update_environment(environment_id, user, **kwargs)
            return _serialize_domain_environment(environment)
        except Exception as exc:
            raise _translate_environment_error(exc) from exc
    require_admin(user)
...
async def delete_environment(
    ...
    if domain is not None:
        ...
        try:
            domain.disable_environment(environment_id, user)
        except Exception as exc:
            raise _translate_environment_error(exc) from exc
        return None
    require_admin(user)
...
async def detect_environment(
    ...
    if domain is not None:
        ...
        try:
            ...
            raise _translate_environment_error(exc) from exc
        # Detection is observation data, not part of the durable registry.
        # Until the runtime detection adapter is installed, do not fall back
        # to the in-memory registry and accidentally mutate legacy state.
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail="Environment detection is not available through the v2 runtime adapter",
        )
    service = _get_environment_service(request)
