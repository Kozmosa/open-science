---
title: 部署概览
description: AINRF 生产部署前置条件、首次登录、安全检查清单、故障排查与环境变量参考。
---

从零将 AINRF 部署到生产环境或实验室服务器的完整指南。覆盖裸机（systemd）和容器（Docker Compose）两种主要部署方式。

import { Card, CardGrid } from '@astrojs/starlight/components';

<CardGrid>
  <Card title="裸机部署" href="/scholar-agent/deployment/bare-metal/">
    一键脚本安装到 Ubuntu/Debian，systemd 管理服务。
  </Card>
  <Card title="Docker Compose" href="/scholar-agent/deployment/docker/">
    标准版、GPU 版、CPU-only 版三种容器部署方案。
  </Card>
  <Card title="Kubernetes" href="/scholar-agent/deployment/kubernetes/">
...
:::tip
详细安全架构参考 [安全架构](/scholar-agent/security/) 和 [生产检查清单](/scholar-agent/security/checklist/)。
:::
...
Docker Compose 部署自带 Prometheus + Grafana 监控栈。详见 [监控栈](/scholar-agent/observability/monitoring-stack/)。
