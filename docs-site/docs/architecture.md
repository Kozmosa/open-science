...
- Issue #76 将已完成的 cutover、migration chain 和 legacy Attempt/RuntimeSession authority 退休。fresh-install baseline anchor 是 `agentic_researcher=33`、`auth=7`、`literature=7`、`terminal=1`；正常 startup 随后应用 `src/ainrf/db/migrations/current.py` 中高于 anchor 的 forward migrations，当前最终支持版本是 `agentic_researcher=38`、`auth=10`、`literature=11`、`terminal=1`。正常 startup 只注册该 migration module 和 `src/ainrf/db/baselines/*.sql`。
- Agentic migrations 034–038 依次收紧 next-Turn cancellation guard、补齐 context snapshot provenance、在 Conversation authority 完整后删除 `tasks.status` shadow、仅在空表时退休无 producer 的 runtime approvals，并在拒绝任何非 canonical 历史 row 后为 Task researcher/harness runtime type 建立 durable CHECK authority。Literature migrations 008–011 依次退休旧 task saga artifact、补齐 `literature_api_attempts` durable lifecycle/evidence guard、删除无 caller 的 `legacy_subscription_id` topic mapping，并在逐行证明 canonical intent 等价后删除 write-only research-Task link mirror；`literature_api_attempts` 仍是外部调用的正式 durable attempt 记录。
- Auth migration 008 在验证 event version 连续且最新事件与 `environment_access` 当前授权 authority 完全一致后，退休无产品或管理 reader 的 grant event ledger；当前 grant row 继续保存单调版本、active/revoked tombstone、actor、reason、quota 与时间信息。Auth migration 009 退休已被 Domain `project_members` 取代的旧 collaborator 表：空表安全删除，任何非空历史 row 都 fail-closed 保留在 version 8，等待人工归档或映射决策。Auth migration 010 在拒绝任何非 canonical 历史 row 后，为 `users.role/status` 建立与 Auth Domain enums 一致的 durable CHECK authority。
...
| `/api/tasks/{task_id}/fork` legacy one-shot Task fork | Conversation Domain / Task HTTP Adapter | WebUI caller 已迁移至 `/api/tasks/{task_id}/fork-preview` → `/api/tasks/{task_id}/fork-preview/{preview_id}/confirm`，由 feature adapter 完成 preview、显式确认与 target Task canonical summary 读取；legacy route 仍继续以旧 payload 映射 canonical Task + Submission | fail-closed 保留；caller 已迁移但 legacy route 未获独立删除批准，仍需完整 caller、generated transport、contract tests 与 release staging 验收后再评估删除 |
