...
- Issue #76 将已完成的 cutover、migration chain 和 legacy Attempt/RuntimeSession authority 退休。fresh-install baseline anchor 是 `agentic_researcher=33`、`auth=7`、`literature=7`、`terminal=1`；正常 startup 随后应用 `src/ainrf/db/migrations/current.py` 中高于 anchor 的 forward migrations，当前最终支持版本是 `agentic_researcher=37`、`auth=7`、`literature=10`、`terminal=1`。正常 startup 只注册该 migration module 和 `src/ainrf/db/baselines/*.sql`。
- Agentic migrations 034–037 依次收紧 next-Turn cancellation guard、补齐 context snapshot provenance、在 Conversation authority 完整后删除 `tasks.status` shadow，并仅在空表时退休无 producer 的 runtime approvals。Literature migrations 008–010 依次退休旧 task saga artifact、补齐 `literature_api_attempts` durable lifecycle/evidence guard，并删除无 caller 的 `legacy_subscription_id` topic mapping；`literature_api_attempts` 仍是外部调用的正式 durable attempt 记录。
