...
- Auth migration 008 在验证 event version 连续且最新事件与 `environment_access` 当前授权 authority 完全一致后，退休无产品或管理 reader 的 grant event ledger；当前 grant row 继续保存单调版本、active/revoked tombstone、actor、reason、quota 与时间信息。
